...
typedef struct {
  ...
  BrkAnimation *reorder_animation;
  gboolean reorder_ignore_bounds;

  double appear_progress;
  BrkAnimation *appear_animation;
...
struct _BrkTabBox
{
  ...
  BrkTabPage *detached_page;
  int detached_index;
  TabInfo *reorder_placeholder;
  BrkTabPage *placeholder_page;
...
G_DEFINE_FINAL_TYPE_WITH_CODE (BrkTabBox, brk_tab_box, GTK_TYPE_WIDGET,
...
static inline int
get_tab_position (BrkTabBox *self,
                  TabInfo   *info,
                  gboolean   final)
{
...
find_tab_info_at (BrkTabBox *self,
                  double     x)
{
  GList *l;
...
static inline GList *
find_link_for_page (BrkTabBox  *self,
                    BrkTabPage *page)
{
  GList *l;
...
static inline TabInfo *
find_info_for_page (BrkTabBox  *self,
                    BrkTabPage *page)
{
...
static GList *
find_nth_alive_tab (BrkTabBox *self,
                    guint      position)
{
  GList *l;
...
static int
get_base_tab_width (BrkTabBox *self,
...
static int
predict_tab_width (BrkTabBox *self,
                   TabInfo   *info,
...
static int
calculate_tab_offset (BrkTabBox *self,
                      TabInfo   *info,
                      gboolean target)
{
  int width;
...
static void
get_visible_range (BrkTabBox *self,
                   int       *lower,
                   int       *upper)
{
  int min = SPACING;
...
static void
resize_animation_done_cb (BrkTabBox *self)
{
  self->end_padding = 0;
...
static void
set_tab_resize_mode (BrkTabBox     *self,
                     TabResizeMode  mode)
{
  gboolean notify;
  ...
  if (mode == TAB_RESIZE_NORMAL) {
    ...
    brk_animation_play (self->resize_animation);
  }
...
update_visible (BrkTabBox *self)
{
  ...
  for (l = self->tabs; l; l = l->next) {
    ...
    brk_tab_set_fully_visible (info->tab,
...
static double
get_scroll_animation_value (BrkTabBox *self)
{
  double to, value;
  ...
  value = brk_animation_get_value (self->scroll_animation);

  return round (brk_lerp (self->scroll_animation_from, to, value));
}

static void
drop_switch_timeout_cb (BrkTabBox *self)
{
  self->drop_switch_timeout_id = 0;
  brk_tab_view_set_selected_page (self->view,
                                  self->drop_target_tab->page);
}

static void
set_drop_target_tab (BrkTabBox *self,
                     TabInfo   *info)
{
...
static void
adjustment_value_changed_cb (BrkTabBox *self)
{
  double value = gtk_adjustment_get_value (self->adjustment);
                ...
                guint      duration)
{
  ...
  brk_timed_animation_set_duration (BRK_TIMED_ANIMATION (self->scroll_animation),
                                    duration);
  brk_animation_play (self->scroll_animation);
}

static void
animate_scroll_relative (BrkTabBox *self,
                         double     delta,
                         guint      duration)
{
...
static gboolean
scroll_to_tab_full (BrkTabBox *self,
                    TabInfo   *info,
                    int        pos,
                    guint      duration,
...
static void
scroll_animation_done_cb (BrkTabBox *self)
{
...
static void
set_hadjustment (BrkTabBox     *self,
                 GtkAdjustment *adjustment)
{
...
static void
start_reordering (BrkTabBox *self,
                  TabInfo   *info)
{
...
static void
reorder_animation_done_cb (BrkTabBox *self)
{
...
static void
animate_reordering (BrkTabBox *self,
                    TabInfo   *dest_tab)
{
  ...
  self->reorder_animation =
    brk_timed_animation_new (GTK_WIDGET (self), 0, 1,
  ...
  brk_animation_play (self->reorder_animation);
...
static void
animate_reorder_offset (BrkTabBox *self,
                        TabInfo   *info,
                        double     offset)
{
  gboolean is_rtl = gtk_widget_get_direction (GTK_WIDGET (self)) == GTK_TEXT_DIR_RTL;
  ...
  info->reorder_animation =
    brk_timed_animation_new (GTK_WIDGET (self), start_offset, offset,
                             REORDER_ANIMATION_DURATION, target);

  g_signal_connect_swapped (info->reorder_animation, "done",
                            G_CALLBACK (reorder_offset_animation_done_cb), info);

  brk_animation_play (info->reorder_animation);
}

static void
reset_reorder_animations (BrkTabBox *self)
{
...
static void
page_reordered_cb (BrkTabBox  *self,
                   BrkTabPage *page,
                   int         index)
{
  GList *link;
...
static void
update_drag_reodering (BrkTabBox *self)
{
  gboolean is_rtl;
                    ...
                    BrkTabBox     *self)
{
  ...
  autoscroll_factor = brk_easing_ease (BRK_EASE_IN_CUBIC,
                                       autoscroll_factor);
...
static void
start_drag_reodering (BrkTabBox *self,
                      TabInfo   *info,
                      double     x,
                      double     y)
{
...
end_drag_reodering (BrkTabBox *self)
{
  ...
  if (!self->indirect_reordering) {
    ...
    brk_tab_view_reorder_page (self->view, self->reordered_tab->page, index);
...
static void
reorder_begin_cb (BrkTabBox  *self,
                  double      start_x,
                  double      start_y,
                  GtkGesture *gesture)
{
...
static gboolean
check_dnd_threshold (BrkTabBox *self,
                     double     x,
                     double     y)
{
  int threshold;
...
static void begin_drag (BrkTabBox *self,
                        GdkDevice *device);

static void
reorder_update_cb (BrkTabBox  *self,
                   double      offset_x,
                   double      offset_y,
                   GtkGesture *gesture)
{
  ...
  if (self->dragging) {
    brk_tab_view_set_selected_page (self->view, self->pressed_tab->page);
...
static void
reset_focus (BrkTabBox *self)
{
  gtk_widget_set_focus_child (GTK_WIDGET (self), NULL);
}

static void
select_page (BrkTabBox  *self,
             BrkTabPage *page)
{
  if (!page) {
...
static gboolean
extra_drag_drop_cb (BrkTab       *tab,
                    GValue       *value,
                    GdkDragAction current_action,
                    BrkTabBox    *self)
{
  gboolean ret = GDK_EVENT_PROPAGATE;
  BrkTabPage *page = brk_tab_get_page (tab);
...
extra_drag_value_cb (BrkTab    *tab,
                     GValue    *value,
                     BrkTabBox *self)
{
...
static TabInfo *
create_tab_info (BrkTabBox  *self,
                 BrkTabPage *page)
{
  TabInfo *info;
  ...
  info->pos = -1;
  info->width = -1;
  info->container = brk_gizmo_new_with_role ("tabboxchild",
  ...
  brk_tab_set_page (info->tab, page);
  brk_tab_set_inverted (info->tab, self->inverted);
  brk_tab_setup_extra_drop_target (info->tab,
...
static void
page_attached_cb (BrkTabBox  *self,
                  BrkTabPage *page,
                  int         position)
{
  BrkAnimationTarget *target;
...
static void
page_detached_cb (BrkTabBox  *self,
                  BrkTabPage *page)
{
  BrkAnimationTarget *target;
  TabInfo *info;
  ...
  target = brk_callback_animation_target_new ((BrkAnimationTargetFunc)
                                              ...
                                              info, NULL);
  info->appear_animation =
    brk_timed_animation_new (GTK_WIDGET (self), info->appear_progress, 0,
...
#define BRK_TYPE_TAB_BOX_ROOT_CONTENT (brk_tab_box_root_content_get_type ())

G_DECLARE_FINAL_TYPE (BrkTabBoxRootContent, brk_tab_box_root_content, BRK, TAB_BOX_ROOT_CONTENT, GdkContentProvider)

struct _BrkTabBoxRootContent
{
...
G_DEFINE_FINAL_TYPE (BrkTabBoxRootContent, brk_tab_box_root_content, GDK_TYPE_CONTENT_PROVIDER)

static GdkContentFormats *
brk_tab_box_root_content_ref_formats (GdkContentProvider *provider)
{
  return gdk_content_formats_new ((const char *[1]) { "application/x-rootwindow-drop" }, 1);
}

static void
brk_tab_box_root_content_write_mime_type_async (GdkContentProvider  *provider,
                                                ...
                                                gpointer             user_data)
{
  BrkTabBoxRootContent *self = BRK_TAB_BOX_ROOT_CONTENT (provider);
  GTask *task;
  ...
  g_task_set_source_tag (task, brk_tab_box_root_content_write_mime_type_async);
...
static gboolean
brk_tab_box_root_content_write_mime_type_finish (GdkContentProvider  *provider,
                                                 GAsyncResult        *result,
                                                 GError             **error)
{
...
static void
brk_tab_box_root_content_finalize (GObject *object)
{
  BrkTabBoxRootContent *self = BRK_TAB_BOX_ROOT_CONTENT (object);

  g_clear_object (&self->tab_box);

  G_OBJECT_CLASS (brk_tab_box_root_content_parent_class)->finalize (object);
}

static void
brk_tab_box_root_content_class_init (BrkTabBoxRootContentClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GdkContentProviderClass *provider_class = GDK_CONTENT_PROVIDER_CLASS (klass);

  object_class->finalize = brk_tab_box_root_content_finalize;

  provider_class->ref_formats = brk_tab_box_root_content_ref_formats;
  provider_class->write_mime_type_async = brk_tab_box_root_content_write_mime_type_async;
  provider_class->write_mime_type_finish = brk_tab_box_root_content_write_mime_type_finish;
}

static void
brk_tab_box_root_content_init (BrkTabBoxRootContent *self)
{
}

static GdkContentProvider *
brk_tab_box_root_content_new (BrkTabBox *tab_box)
{
  BrkTabBoxRootContent *self = g_object_new (BRK_TYPE_TAB_BOX_ROOT_CONTENT, NULL);
...
static int
calculate_placeholder_index (BrkTabBox *self,
                             int        x)
{
...
static void
insert_placeholder (BrkTabBox  *self,
                    BrkTabPage *page,
                    int         pos)
{
  ...
  if (info) {
    ...
    brk_tab_set_dragging (info->tab, TRUE);
  ...
  target = brk_callback_animation_target_new ((BrkAnimationTargetFunc)
                                              ...
                                              info, NULL);
  info->appear_animation =
    brk_timed_animation_new (GTK_WIDGET (self), initial_progress, 1,
                             OPEN_ANIMATION_DURATION, target);
...
static void
replace_placeholder (BrkTabBox  *self,
                     BrkTabPage *page)
{
  TabInfo *info = self->reorder_placeholder;
  ...
  brk_tab_set_dragging (info->tab, FALSE);
...
remove_animation_done_cb (TabInfo *info)
{
  ...
  if (!self->can_remove_placeholder) {
    brk_tab_set_page (info->tab, self->placeholder_page);
...
static void
remove_placeholder_scroll_cb (BrkTabBox *self)
{
...
static void
remove_placeholder (BrkTabBox *self)
{
  TabInfo *info = self->reorder_placeholder;
  BrkAnimationTarget *target;
...
get_source_tab_box (GtkDropTarget *target)
{
  ...
  return BRK_TAB_BOX (g_object_get_data (G_OBJECT (drag),
                      "brk-tab-bar-drag-origin"));
}

static void
do_drag_drop (BrkTabBox *self,
...
static void
detach_into_new_window (BrkTabBox *self)
{
  BrkTabPage *page;
  BrkTabView *new_view;
...
static gboolean
is_view_in_the_same_group (BrkTabBox  *self,
          ...
          gboolean   success)
{
  ...
  if (!success) {
    brk_tab_view_attach_page (self->view,
                              self->detached_page,
                              self->detached_index);
...
static void
tab_drop_performed_cb (BrkTabBox *self,
                       GdkDrag   *drag)
{
...
static void
tab_dnd_finished_cb (BrkTabBox *self,
                     GdkDrag   *drag)
{
...
static void
tab_drag_cancel_cb (BrkTabBox           *self,
...
static void
create_drag_icon (BrkTabBox *self,
                  GdkDrag   *drag)
{
  DragIcon *icon;
  BrkAnimationTarget *target;
  ...
  icon->resize_animation =
    brk_timed_animation_new (GTK_WIDGET (icon->tab), 0, 0,
...
static void
resize_drag_icon (BrkTabBox *self,
                  int        width)
{
  ...
  brk_timed_animation_set_value_from (BRK_TIMED_ANIMATION (icon->resize_animation),
                                      icon->width);
  brk_timed_animation_set_value_to (BRK_TIMED_ANIMATION (icon->resize_animation),
                                    width);
  brk_animation_play (icon->resize_animation);
}

static void
begin_drag (BrkTabBox *self,
            GdkDevice *device)
{
  ...
  content = gdk_content_provider_new_union ((GdkContentProvider *[2]) {
                                              brk_tab_box_root_content_new (self),
                                              gdk_content_provider_new_typed (BRK_TYPE_TAB_PAGE, detached_info->page)
                                            }, 2);

  drag = gdk_drag_begin (surface, device, content, GDK_ACTION_MOVE,
                         self->reorder_x, self->reorder_y);

  g_object_set_data (G_OBJECT (drag), "brk-tab-bar-drag-origin", self);
  ...
  self->detached_index = brk_tab_view_get_page_position (self->view, self->detached_page);

  brk_tab_view_detach_page (self->view, self->detached_page);
...
static GdkDragAction
tab_drag_enter_motion_cb (BrkTabBox     *self,
                          double         x,
                          double         y,
                          GtkDropTarget *target)
{
  ...
  if (!self->reorder_placeholder || !self->reorder_placeholder->page) {
    ...
    brk_tab_set_inverted (source_tab_box->drag_icon->tab, self->inverted);
...
static void
tab_drag_leave_cb (BrkTabBox     *self,
                   GtkDropTarget *target)
{
  BrkTabBox *source_tab_box;
...
static gboolean
tab_drag_drop_cb (BrkTabBox     *self,
                  const GValue  *value,
                  double         x,
                  double         y,
                  GtkDropTarget *target)
...
static gboolean
view_drag_drop_cb (BrkTabBox      *self,
                   const GValue  *value,
                   double         x,
                   double         y,
                   GtkDropTarget *target)
...
static void
reset_drop_target_tab_cb (BrkTabBox *self)
{
...
static void
drag_enter_motion_cb (BrkTabBox               *self,
                      double                   x,
                      double                   y,
                      GtkDropControllerMotion *controller)
{
...
static void
reset_setup_menu_cb (BrkTabBox *self)
{
...
static void
long_pressed_cb (BrkTabBox  *self,
                 double      x,
                 double      y,
                 GtkGesture *gesture)
{
             ...
             GtkGesture *gesture)
{
  ...
  if (button == GDK_BUTTON_MIDDLE) {
    brk_tab_view_close_page (self->view, info->page);
...
static void
measure_tab_box (BrkTabBox      *self,
                 GtkOrientation  orientation,
                 int            *minimum,
                 int            *natural,
                 gboolean        animated)
...
static void
brk_tab_box_measure (GtkWidget      *widget,
                     GtkOrientation  orientation,
                     int             for_size,
                     int            *minimum,
                     int            *natural,
...
static void
brk_tab_box_size_allocate (GtkWidget *widget,
                           int        width,
                           int        height,
                           int        baseline)
{
  ...
  brk_tab_box_measure (widget, GTK_ORIENTATION_HORIZONTAL, -1,
  ...
  if (self->scroll_animation_done) {
    ...
    self->scroll_animation_done = FALSE;
    brk_animation_reset (self->scroll_animation);
  }
...
static void
brk_tab_box_snapshot (GtkWidget   *widget,
                      GtkSnapshot *snapshot)
{
  BrkTabBox *self = BRK_TAB_BOX (widget);
  double value = gtk_adjustment_get_value (self->adjustment);
...
static gboolean
brk_tab_box_focus (GtkWidget        *widget,
                   GtkDirectionType  direction)
{
  BrkTabBox *self = BRK_TAB_BOX (widget);
...
static void
brk_tab_box_unmap (GtkWidget *widget)
{
  BrkTabBox *self = BRK_TAB_BOX (widget);
  ...
  GTK_WIDGET_CLASS (brk_tab_box_parent_class)->unmap (widget);
}

static void
brk_tab_box_direction_changed (GtkWidget        *widget,
...
static void
brk_tab_box_dispose (GObject *object)
{
  ...
  self->tab_bar = NULL;
  brk_tab_box_set_view (self, NULL);
  ...
  G_OBJECT_CLASS (brk_tab_box_parent_class)->dispose (object);
}
...
brk_tab_box_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (brk_tab_box_parent_class)->finalize (object);
}

static void
brk_tab_box_get_property (GObject    *object,
                          guint       prop_id,
                          GValue     *value,
                          GParamSpec *pspec)
{
...
static void
brk_tab_box_set_property (GObject      *object,
                          guint         prop_id,
                          const GValue *value,
                          GParamSpec   *pspec)
{
  ...
  case PROP_VIEW:
    brk_tab_box_set_view (self, g_value_get_object (value));
    break;
...
static void
brk_tab_box_class_init (BrkTabBoxClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose = brk_tab_box_dispose;
  object_class->finalize = brk_tab_box_finalize;
  object_class->get_property = brk_tab_box_get_property;
  object_class->set_property = brk_tab_box_set_property;

  widget_class->measure = brk_tab_box_measure;
  widget_class->size_allocate = brk_tab_box_size_allocate;
  widget_class->snapshot = brk_tab_box_snapshot;
...
static void
brk_tab_box_init (BrkTabBox *self)
{
  GtkEventController *controller;
  BrkAnimationTarget *target;
  GtkWidget *widget;
  ...
  controller = GTK_EVENT_CONTROLLER (gtk_drop_target_new (BRK_TYPE_TAB_PAGE, GDK_ACTION_MOVE));
  ...
  widget = brk_gizmo_new ("indicator", NULL, NULL, NULL, NULL, NULL, NULL);
...
void
brk_tab_box_set_view (BrkTabBox  *self,
                      BrkTabView *view)
{
...
void
brk_tab_box_attach_page (BrkTabBox  *self,
                         BrkTabPage *page,
                         int         position)
{
  g_return_if_fail (BRK_IS_TAB_BOX (self));
...
void
brk_tab_box_detach_page (BrkTabBox  *self,
                         BrkTabPage *page)
{
  g_return_if_fail (BRK_IS_TAB_BOX (self));
  g_return_if_fail (BRK_IS_TAB_PAGE (page));
...
void
brk_tab_box_select_page (BrkTabBox  *self,
                         BrkTabPage *page)
{
  g_return_if_fail (BRK_IS_TAB_BOX (self));
...
gboolean
brk_tab_box_is_page_focused (BrkTabBox  *self,
                             BrkTabPage *page)
{
  TabInfo *info;

  g_return_val_if_fail (BRK_IS_TAB_BOX (self), FALSE);
...
void
brk_tab_box_setup_extra_drop_target (BrkTabBox     *self,
...
gboolean
brk_tab_box_get_expand_tabs (BrkTabBox *self)
{
  g_return_val_if_fail (BRK_IS_TAB_BOX (self), FALSE);
...
void
brk_tab_box_set_expand_tabs (BrkTabBox *self,
                             gboolean   expand_tabs)
{
  g_return_if_fail (BRK_IS_TAB_BOX (self));
...
gboolean
brk_tab_box_get_inverted (BrkTabBox *self)
{
  g_return_val_if_fail (BRK_IS_TAB_BOX (self), FALSE);
...
void
brk_tab_box_set_inverted (BrkTabBox *self,
                          gboolean   inverted)
{
  GList *l;
  ...
  for (l = self->tabs; l; l = l->next) {
    ...
    brk_tab_set_inverted (info->tab, inverted);
  }
}

gboolean
brk_tab_box_get_extra_drag_preload (BrkTabBox *self)
{
  g_return_val_if_fail (BRK_IS_TAB_BOX (self), FALSE);
...
void
brk_tab_box_set_extra_drag_preload (BrkTabBox *self,
                                    gboolean   preload)
{
  GList *l;
  ...
  for (l = self->tabs; l; l = l->next) {
    ...
    brk_tab_set_extra_drag_preload (info->tab, preload);
  }
}
