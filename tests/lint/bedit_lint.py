...
SOURCE_ROOT = PROJECT_ROOT / pathlib.Path("bedit")
INCLUDE_ROOT = PROJECT_ROOT / pathlib.Path("bedit")
...
def enumerate_object_paths():
    paths = [object_path for object_path in BUILD_ROOT.glob("bedit/**/*.o")]
    yield from sorted(paths)
...
def header_paths_for_source_path(source_path, /):
    assert is_source_path(source_path), source_path
    ...
    private_header_path = (
        INCLUDE_ROOT
        / source_path.relative_to(SOURCE_ROOT).parent
        / (source_path.stem + "-private.h")
    )
    if private_header_path.exists():
        paths.append(private_header_path)
...
def source_path_for_header_path(header_path, /):
    ...
    source_path = (
        ...
        / (header_path.stem.replace("-private", "")  + ".c")
    )
...
def include_dirs(source_path):
    return [
        ...
        (BUILD_ROOT / pathlib.Path(arg[len("-isystem"):])).resolve()
        for arg in get_args(source_path)
